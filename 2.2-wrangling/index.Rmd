...
qplot(logCPM, estimate, colour=significant, data=tet, alpha=I(.1)) + facet_wrap(~significant)
```
...
For the previous example, produce a Volcano plot for each of the model terms, i.e. plot estimates on x by log(p.values) on y. Are there differences visible between the terms?
...
```{r}
bbfit <- tidy(fit)
ggplot(data=bbfit, aes(x=term, y=estimate, color=log(p.value), group=gene)) +
  geom_line()
```

```{r eval=FALSE, echo=FALSE}
bbfit_m <- bbfit %>% spread(term, estimate) %>%
  rename(interaction=`typemutant:trttreatment`) %>%
  filter(!is.na(interaction)) %>% filter(!is.na(trttreatment))
ggpairs(bbfit_m, columns=5:7, ggplot2::aes(colour="p.value"))
```
